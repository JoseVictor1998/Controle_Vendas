...
-- 1. FILA DA ARTE: O que o Designer precisa fazer (Pedidos novos ou em análise)
...
PI.Observacao_Tecnica AS Detalhes,
ISNULL(SA.Nome, 'Sem Arte') AS Situacao_Arte  -- Se não tiver arquivo ainda, mostra 'Sem Arte'
FROM Pedido P
...
WHERE P.Status_ID IN (1, 2, 3);-- 1:Criado, 2:Aguardando Arte, 3:Em Análise


-- 2. FILA DE IMPRESSÃO: O seu painel (Só o liberado)
...
WHERE P.Status_ID = 4;-- 4: Arte Aprovada


-- 3. FILA DE ACABAMENTO: O que já saiu da impressão
...
WHERE P.Status_ID IN (5, 6);-- 5: Em Produção, 6: Finalizado


-- 4. MONITORAMENTO GLOBAL: Ver tudo sem filtros (O "onde está o pedido?")
...
-- 5. DASHBOARD PARA O DONO: Resumo de ocupação por setor
